...
this.state.count = this.state.count + 1;  // Class component
...
// Can also use props
this.setState((prevState, props) => ({
  total: prevState.count + props.increment
}));
```
...
```javascript
// Use updater when new state depends on old state:
setCount(c => c + 1);              // ✅ Yes
setItems(items => [...items, item]); // ✅ Yes

// Direct value when state is independent:
setName('John');                   // ✅ Fine
setTheme('dark');                  // ✅ Fine
```
...
### OQ14: What is the purpose of super constructor with props argument?
...
**Class Components Only (Legacy):**

```javascript
class MyComponent extends React.Component {
  constructor(props) {
    super(props); // ← Why is this needed?
    this.state = { count: 0 };
  }
}
```
...
#### 1. **Calls Parent Class Constructor**
```javascript
class Component {
  constructor(props) {
    this.props = props; // Parent initializes props
  }
}
...
#### 2. **Makes `this.props` Available in Constructor**
```javascript
// ❌ Without super(props):
class MyComponent extends React.Component {
  constructor(props) {
    ...
    console.log(this.props); // undefined! ⚠️
    // But props parameter is still accessible:
...
constructor(props) {
  ...
  // Common use case: Initialize state from props
  this.state = {
...
**Note:**
```javascript
// In modern React (function components + hooks):
// You don't need constructors at all!

function MyComponent({ initialColor }) {
...
### OQ20 & OQ22: setState in constructor & props in initial state
...
class MyComponent extends React.Component {
  constructor(props) {
    ...
    // ❌ Wrong - use direct assignment instead
...
**Why you can't use setState in constructor:**
- Component not mounted yet
- setState triggers re-render, but nothing to re-render
- Constructor is for initialization only
- Use direct assignment: `this.state = { }`

**OQ22: What happens if you use props in initial state?**

```javascript
// ⚠️ Antipattern - state won't update when props change
class MyComponent extends React.Component {
  constructor(props) {
    super(props);

    // State initialized once, never updates
    this.state = {
      color: props.color  // Initial value from prop
    };
  }
}

// If parent changes prop:
<MyComponent color="red" />  // state.color = "red"
<MyComponent color="blue" /> // state.color still "red"! ⚠️
```

**When it's OK:**
```javascript
// ✅ Intentionally use prop as INITIAL value only
class MyComponent extends React.Component {
  constructor(props) {
    super(props);

    this.state = {
      // Using prop as seed value, then managing internally
      color: props.initialColor
    };
  }

  // Prop name makes it clear it's initial value only
}
```
...
```javascript
// Option 1: Use prop directly (no state needed)
function MyComponent({ color }) {
  return <div style={{ color }}>{color}</div>;
}

// Option 2: Sync state with props using useEffect
function MyComponent({ color }) {
  const [currentColor, setCurrentColor] = useState(color);

  useEffect(() => {
    setCurrentColor(color); // Update when prop changes
  }, [color]);

  return <div style={{ color: currentColor }}>{currentColor}</div>;
}

// Option 3: Use key to reset component
<MyComponent key={color} color={color} />
```
...
**Already covered in OQ14, quick comparison:**

```javascript
// Without props:
constructor(props) {
  super();
  console.log(this.props); // undefined in constructor
}
...
**Modern React:**
```javascript
// Don't need constructors with hooks!
function MyComponent(props) {
  console.log(props); // Just use it
}
```
...
**Class Components - Automatic Merging:**
```javascript
class MyComponent extends React.Component {
  state = {
    name: 'John',
  ...
  updateName = () => {
    // Only updates 'name', keeps age and email
    this.setState({ name: 'Jane' });
  }

  // Result:
  // { name: 'Jane', age: 30, email: 'john@example.com' }
  // age and email are preserved!
}
```
